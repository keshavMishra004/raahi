...
const TABS = ["Company", "Legal", "Finance", "Documents", "Extras"];
...
export default function CompanyPage() {
  ...
  // Certifications, insurance, regulatory, maintenance
  const [certifications, setCertifications] = useState([]); // { name, certificateUrl, validFrom, validTo }
  ...
  const [maintenance, setMaintenance] = useState({
    ...
    nextDue: "",
    statusNotes: "",
    maintenanceRecords: [], // { date, performedBy, notes, documentUrl }
  });
  ...
  const addCertification = () => setCertifications((p) => [...p, { name: "", certificateUrl: "", validFrom: "", validTo: "" }]);
  const updateCertification = (idx, field, value) => setCertifications((p) => { const arr = [...p]; arr[idx] = { ...arr[idx], [field]: value }; return arr; });
  const removeCertification = (idx) => setCertifications((p) => p.filter((_, i) => i !== idx));
  ...
  const saveData = async (goNext = false) => {
    ...
    try {
      const payload = {
        company: {
          ...company,
          logo: company.logo ? { name: company.logo.name } : null, // send only metadata for logo, not the File object
        },
        location,
        contactInfo,
        businessInfo,
        ...
        maintenance,
        // finance: accountNumber now includes IBAN where applicable
        finance,
        documents: {
            ...
            <div className="mt-1 flex items-center gap-3">
              ...
              {company.logo ? (
                <div className="text-sm text-gray-600">
                  <span className="font-medium">{company.logo.name}</span>
                  {/* optional small preview if you want */}
                </div>
              ) : (
                <div className="text-sm text-gray-400">No logo selected</div>
              )}
            </div>
          </div>
  ...
  const renderLegalTab = () => (
    <div className="space-y-4">
      {card("Legal & Business Info", (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            {/* replaced plain input with select using COUNTRIES (India on top) */}
          ...
          <div className="mt-3">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-medium text-gray-700">Business Licenses</h4>
              <button type="button" onClick={addBusinessLicenseEntry} className="text-sm text-blue-600 cursor-pointer">+ Add</button>
            </div>
            <div className="space-y-2 mt-2">
              {businessInfo.businessLicenses.map((b, idx) => (
                <div key={idx} className="p-3 border rounded-md bg-gray-50">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    <input placeholder="Name" value={b.name || ""} onChange={(e) => updateBusinessLicense(idx, "name", e.target.value)} className="px-2 py-1 rounded border" />
                    <input placeholder="Issued By" value={b.issuedBy || ""} onChange={(e) => updateBusinessLicense(idx, "issuedBy", e.target.value)} className="px-2 py-1 rounded border" />
                    <input type="date" value={b.issuedDate || ""} onChange={(e) => updateBusinessLicense(idx, "issuedDate", e.target.value)} className="px-2 py-1 rounded border" />
                  </div>
                  <div className="flex items-center justify-between mt-2">
                    <input type="date" value={b.expiryDate || ""} onChange={(e) => updateBusinessLicense(idx, "expiryDate", e.target.value)} className="px-2 py-1 rounded border" />
                    <button onClick={() => removeBusinessLicense(idx)} className="text-red-600 text-sm cursor-pointer">Remove</button>
                  </div>
                </div>
              ))}
              {businessInfo.businessLicenses.length === 0 && <p className="text-sm text-gray-400">No licenses added</p>}
            </div>
          </div>
        </>
      ))}
      ...
      {card("Certifications", (
        <>
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">Add certifications like DGCA, IATA, FAA etc.</p>
            <button type="button" onClick={addCertification} className="text-sm text-blue-600 cursor-pointer">+ Add</button>
          </div>
          <div className="space-y-2 mt-2">
            {certifications.map((c, idx) => (
              <div key={idx} className="p-3 border rounded-md bg-gray-50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  <input placeholder="Name" value={c.name} onChange={(e) => updateCertification(idx, "name", e.target.value)} className="px-2 py-1 rounded border" />
                  <input placeholder="Certificate URL" value={c.certificateUrl} onChange={(e) => updateCertification(idx, "certificateUrl", e.target.value)} className="px-2 py-1 rounded border" />
                  <input type="date" value={c.validFrom} onChange={(e) => updateCertification(idx, "validFrom", e.target.value)} className="px-2 py-1 rounded border" />
                </div>
                <div className="flex items-center justify-between mt-2">
                  <input type="date" value={c.validTo} onChange={(e) => updateCertification(idx, "validTo", e.target.value)} className="px-2 py-1 rounded border" />
                  <button onClick={() => removeCertification(idx)} className="text-red-600 text-sm cursor-pointer">Remove</button>
                </div>
              </div>
            ))}
            {certifications.length === 0 && <p className="text-sm text-gray-400">No certifications added</p>}
          </div>
        </>
      ))}
      ...
      {card("Insurance", (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <input placeholder="Policy Document URL" value={insurance.policyDocumentUrl} onChange={(e) => updateInsurance("policyDocumentUrl", e.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2" />
          <input placeholder="Coverage Details" value={insurance.coverageDetails} onChange={(e) => updateInsurance("coverageDetails", e.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2" />
          <input type="date" value={insurance.validFrom} onChange={(e) => updateInsurance("validFrom", e.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2" />
          <input type="date" value={insurance.validTo} onChange={(e) => updateInsurance("validTo", e.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2" />
        </div>
      ))}
      ...
      {card("Regulatory & Safety", (
        <>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <label className="flex items-center gap-2"><input type="checkbox" checked={regulatory.dgcaAirAmbulanceApproval} onChange={(e) => updateRegulatory("dgcaAirAmbulanceApproval", e.target.checked)} /> DGCA Air Ambulance Approval</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
            ...
            <input placeholder="Maintenance Organization Approval" value={regulatory.maintenanceOrganizationApproval} onChange={(e) => updateRegulatory("maintenanceOrganizationApproval", e.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2" />
          </div>
          <div className="mt-2">
            <label className="block text-sm font-medium text-gray-700">Emergency Equipment (comma separated)</label>
            <input placeholder="Fire extinguisher, ELT, Life vests" value={regulatory.emergencyEquipment} onChange={(e) => updateRegulatory("emergencyEquipment", e.target.value)} className="mt-1 block w-full rounded-md border px-3 py-2" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700">DGCA Compliance Notes</label>
            <textarea value={regulatory.dgcaComplianceNotes} onChange={(e) => updateRegulatory("dgcaComplianceNotes", e.target.value)} rows={3} className="mt-1 block w-full rounded-md border px-3 py-2" />
          </div>
        </>
      ))}

      {card("Maintenance Records", (
        <>
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-600">Track maintenance history</p>
            <button type="button" onClick={addMaintenanceRecord} className="text-sm text-blue-600 cursor-pointer">+ Add record</button>
          </div>
          <div className="space-y-2 mt-2">
            {maintenance.maintenanceRecords.map((m, idx) => (
              <div key={idx} className="p-3 border rounded-md bg-gray-50">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  <input type="date" value={m.date} onChange={(e) => updateMaintenanceRecord(idx, "date", e.target.value)} className="px-2 py-1 rounded border" />
                  <input placeholder="Performed By" value={m.performedBy} onChange={(e) => updateMaintenanceRecord(idx, "performedBy", e.target.value)} className="px-2 py-1 rounded border" />
                  <input placeholder="Document URL" value={m.documentUrl} onChange={(e) => updateMaintenanceRecord(idx, "documentUrl", e.target.value)} className="px-2 py-1 rounded border" />
                </div>
                <textarea placeholder="Notes" value={m.notes} onChange={(e) => updateMaintenanceRecord(idx, "notes", e.target.value)} rows={2} className="mt-2 w-full px-2 py-1 rounded border" />
                <div className="text-right mt-2">
                  <button type="button" onClick={() => removeMaintenanceRecord(idx)} className="text-red-600 text-sm cursor-pointer">Remove</button>
                </div>
              </div>
            ))}
            {maintenance.maintenanceRecords.length === 0 && <p className="text-sm text-gray-400">No maintenance records</p>}
          </div>
        </>
      ))}
    </div>
